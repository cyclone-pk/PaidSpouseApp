...
import 'package:matriapp/Screens/auth/otp_verification.dart';
  ...
  _verificationComplete(
    ...
    if (widget.updateNumber) {
      ...
      FirebaseAuth.instance
          ...
          .then((UserCredentail) async {
        print(UserCredentail.user.uid);
        //snackbar("Success!!! UUID is: " + UserCredentail.user.uid);
        showDialog(
            barrierDismissible: false,
            context: context,
            builder: (_) {
              Future.delayed(Duration(seconds: 2), () async {
                Navigator.pop(context);
                await _login.navigationCheck(UserCredentail.user, context);
              });
              return Center(
                  child: Container(
                      width: 150.0,
